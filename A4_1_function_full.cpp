// A4_1_function_full.cpp : 定义控制台应用程序的入口点。
//
...
void collision(float u[101][101], float v[101][101], float f[9][101][101], float rho[101][101], float w[9], float cx[9], float cy[9], int n, int m, float omega)
{
	float feq[9][101][101];
	float t1, t2;
	int i, j, k;
...
void init(float rho[101][101], float th[101][101], float f[9][101][101], float g[9][101][101], float u[101][101], float v[101][101], int n, int m, float uo, float rhoo, float w[9])
{
	int i, j, k;
	for (j = 0; j <= m; j++)
	{
...
int main()
{
	...
	float x[n + 1], y[m + 1], f[9][n + 1][m + 1], rho[n + 1][m + 1], w[9], u[n + 1][m + 1], v[n + 1][m + 1], g[9][n + 1][m + 1], th[n + 1][m + 1], velocity[n + 1][m + 1];
	float uo, vo, sumvelo, rhoo, rhon, dx, dy, dt, tw, visco, pr, alpha, Re, omega, omegat, ssum, usum, vsum, sum;
	int i, j, k, kk;
